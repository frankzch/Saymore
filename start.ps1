# 一键后台启动 Typeoff。用 venv 里的 pythonw.exe（无黑窗、无控制台），
...
$running = Get-CimInstance Win32_Process -Filter "Name='pythonw.exe'" |
    Where-Object { $_.CommandLine -like "*typeoff.main*" -or $_.CommandLine -like "*voice_input.py*" }
if ($running) { Write-Host "已在运行（PID $($running.ProcessId -join ',')），跳过启动。"; return }

Start-Process $pythonw -ArgumentList "-m","typeoff.main" -WorkingDirectory $here
