...
Saymore 用 PyInstaller 把 Python 运行时 + 依赖打成 onedir 应用,再用 Inno Setup 封成 Windows
...
3. **上传 gguf 到 HuggingFace / ModelScope**,并把仓库 ID 填进 [saymore/runtime_check.py](saymore/runtime_check.py) 顶部:

   ```python
   _HF_REPO = "yourname/Saymore-Qwen3-ASR"
   _MS_REPO = "yourname/Saymore-Qwen3-ASR"
   ```
...
- `dist\Saymore\Saymore.exe` + 一堆依赖(直接双击可跑,用于本机验证)
- `installer\output\Saymore-Setup-1.0.0.exe` (最终安装包,发给用户)

## 打包前会发生什么

`saymore.spec` 会把这些**打进 exe 目录**(占大头):
...
PyInstaller 静态分析漏掉动态导入。打开 `saymore.spec` 找 `hidden = []` 那段,加进去:
...
打包时资源没搬全。检查 `dist\Saymore\` 里对应目录是否存在。不在就在 `saymore.spec` 的
`datas = []` 段加一行。

### 3. `DLL load failed while importing xxx`

某个 native 依赖的 DLL 没跟着走。多见于 sherpa-onnx / onnxruntime。运行 `dist\Saymore\Saymore.exe`
...
检查 `dist\Saymore\rapidocr_onnxruntime\` 目录是否含 `.onnx` 文件。
...
1. `dist\Saymore\Saymore.exe` 直接双击 → 系统托盘出图标 + 悬浮窗出圆环
2. 由于是打包环境,`kws-model / polish_lora / llama-cpp / silero_vad.onnx` 都在 → 需下载
   的两个 gguf 缺失 → 悬浮窗应变红,主界面自动开在「运行环境」tab
3. 手动把两个 gguf 放到 `dist\Saymore\models\Qwen3-ASR-1.7B-GGUF\` 下 → 点「重新检测」→
   全绿 → 点「重启程序」→ 圆环变蓝,可正常语音输入
4. 关窗 → 托盘图标还在 → 右键"退出程序"能干净退
5. 装安装包(`installer\output\Saymore-Setup-*.exe`),重复步骤 1-4
