...
**术语纠正：** 专有名词/常用词识别不准时有三层手段：① `qwen_context_file`（默认 `ai_terms.txt`，一行一词）作为 context 喂给 Qwen3-ASR 做上下文偏置；② **热词自学习**——语音说「发送」时程序先用 UIA 读取输入框里的最终文本（含你手动改过的部分）记入 `typed_history.jsonl`，进入休眠时交给 LLM 切词、累计词频（你改过的词加权 3 倍），高频词自动写入 `hotwords.txt` 并与术语表一起偏置，越用越准（手动按回车发送的不采集）；③ `replacements.json` 做"错→对"后处理替换（纯英文键忽略大小写、按词边界替换）。
